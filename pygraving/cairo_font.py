...
import ctypes as ct
# detect platform: linux or mac (Darwin):
import platform

import cairo

if platform.system() == "Linux":
    _freetype_so_name = "libfreetype.so.6"
    _cairo_so_name = "libcairo.so.2"
elif platform.system() == "Darwin":
    _freetype_so_name = "libfreetype.6.dylib"
    _cairo_so_name = "libcairo.2.dylib"
...
def create_cairo_font_face_for_file(filename, faceindex=0, loadoptions=0):
    ...
    if not _initialized:
        # find shared objects
        _freetype_so = ct.CDLL(_freetype_so_name)
        _cairo_so = ct.CDLL(_cairo_so_name)
